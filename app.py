...
# Logging function (simplistic and insecure)
...
# Routes with additional vulnerabilities
...
def vulnerable_sql():
    ...
    result = db.engine.execute(query)  # SQL Injection risk
...
def command_injection():
    ...
    output = subprocess.check_output(command, shell=True).decode('utf-8')  # Command Injection
...
def insecure_upload():
    ...
    file.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))  # Vulnerability: No file type check
...
# Exposed database connection (direct access without sanitization)
def get_db_connection():
    conn = sqlite3.connect('learning.db')  # Direct SQLite connection
    return conn
